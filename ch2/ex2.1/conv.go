package tempconv

// CToF converts a Celsius temperature to Fahrenheit 
func CToF(c Celisius) Fahrenheit { return Fahrenheit(c*9/5 + 32) }

// CToK converts a Celsius temperature to Kelvin
func CToK(c Celisius) Kelvin { return Kelvin(c + 273.15) }

// FToC converts a Fahrenheit temperature to Celsius 
func FToC(f Fahrenheit) Celisius { return Celisius((f - 32) * 5 / 9) }

// FToK converts a Fahrenheit temperature to Kelvin
func FToK(f Fahrenheit) Kelvin { return Kelvin((f*9/5 + 32) + 273.15) }

// KToC converts a Kelvin temperature to Celsius
func KToC(k Kelvin) Celisius { return Celisius(k - 273.15) }

// KToF converts a Kelvin temperature to Fahrenheit
func KToF(k Kelvin) Fahrenheit { return Fahrenheit((k - 273.15)*9/5+32) }
